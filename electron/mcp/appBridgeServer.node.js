  ...
  switch (method) {
    case 'mind.status':
      return {
        ok: true,
        bridge: 'ready',
        windows: listMindWindows(context),
      };
    ...
    case 'mind.listWindows':
      return listMindWindows(context);
    ...
    case 'mind.closeWindow': {
      const closed = await closeMindWindow(context, params?.windowKey);
      return { ok: closed, windowKey: params?.windowKey || null };
    }
    ...
    case 'mind.closeAllWindows': {
      ...
      for (const item of windows) {
        const closed = await closeMindWindow(context, item.windowKey);
        results.push({ windowKey: item.windowKey, docId: item.docId, ok: closed });
      }
