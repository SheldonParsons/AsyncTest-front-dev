            ...
            <ContextMemu :data="data"
                ...
                " class="tree-node g-unselect" @mouseenter="current_node = data.id" @mouseleave="current_node = -1">
                    ...
                    <div class="expand-icon">
                        <LoadingMini v-if="data.child_type !== 4 && current_loading_node === data.id"></LoadingMini>
                        <el-icon v-if="data.child_type !== 4 && current_loading_node !== data.id" :size="8"
                            color="#606266" :class="node.expanded ? 'private-icon icon-expanded' : 'private-icon'
...
watch(
    ...
    async (newCount) => {
        ...
        } else if (GlobalState.message === "create_ds_under_root") {
            const root_node = find_root();
            current_action_data.value = {
                type: 1,
                child_action_type: "create_ds_under_dir",
                content: {
...
async function action(father: number, action_type: string, data: any, node = null) {
    ...
    if (action_type === "create_ds_under_dir") {
        show_dialog.value = true;
...
function changeMenu(data: any, node: any, event: any, event_object: any) {
    if (data.child_type === 1) {
        ...
        send_message_to_tab("click_ds", data, node);
    } else if (data.child_type === 0) {
        send_message_to_tab("click_ds_root_dir", data, node);
    }
}
