...
interface Question { title: string; description?: string; items: QuestionItem[]; diff?: EditDiff; cascade?: CascadeRow[] }
...
function onSend() {
  if (props.sending || sendDisabled.value) return
...
.send-button:not(:disabled):hover { transform: translateY(-1px); background: #030712; }
.send-button:not(:disabled):active { transform: translateY(0) scale(.98); }
.send-button.is-sending { color: #fff; background: #1f2937; pointer-events: none; }
