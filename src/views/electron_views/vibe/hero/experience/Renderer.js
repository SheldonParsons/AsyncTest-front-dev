...
export default class Renderer
{
    ...
    setInstance()
    {
        ...
        this.instance = new THREE.WebGLRenderer({
            ...
            antialias: false,
            alpha: true, // 画布透明，让底层 HTML 背景视频透过来（粒子叠在视频上）
            // stencil: false,
            // depth: false,
        ...
        // No tone mapping was set anywhere in this project (defaults to
        // THREE.NoToneMapping), which hard-clips any composited value above
        // 1.0 straight to pure white with no falloff curve. Combined with
        // the bloom pass below (threshold 0 = everything blooms, strength
        // 2.2 = very strong), that clipping is what turned the whole frame
        // into flat white blobs with soft gray blur edges — filmic tone
        // mapping rolls highlights off gracefully instead of slamming them
        // to white. See HANDOFF.md "White background — exposure pipeline
        // fix" for the full diagnosis.
        ...
        this.instance.setClearColor(this.clearColor, 0) // alpha 0：透明清屏
        this.instance.setSize(this.sizes.width, this.sizes.height)
    }
    ...
    setPostProcess() {
         ...
         * Passes
         */
        // Render pass —— clearColor 黑 + clearAlpha 0：场景以透明清屏，粒子叠在透明层上，
        // 最终经 UnrealBloomPass（自带 base-copy，见其 render() 的 this.basic）合成到透明画布。
        this.postProcess.renderPass = new RenderPass(this.scene, this.camera.instance, null, 0x000000, 0)

        // Bloom pass
        // Was (strength, radius, threshold) = (2.2, 1.0, 0) — threshold 0
        // means literally every pixel, even near-black, gets picked up by
        // the bright-pass and blurred back in at a very high strength. That
        // (plus no tone mapping — see setInstance()) is what blew the whole
        // frame out to white. threshold 0.2 means only genuinely bright
        // particle cores/highlights bloom; strength 1.0 is still a visible
        // glow without being overpowering. Tune further once you can see it
        // live — this is an exposure/feel call, not something to fully
        // nail from code alone.
