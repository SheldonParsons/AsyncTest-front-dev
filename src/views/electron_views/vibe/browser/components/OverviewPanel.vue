    ...
    <section class="sources-section">
      ...
      <div class="section-list source-list">
        <button v-for="item in sources" :key="item.id" class="source-row" type="button" @click="$emit('open-source', item.id)">
          <div><strong>{{ item.display_name || item.filename }}</strong><small>{{ item.display_kind }} · 提交 #{{ item.commit_seq }} · {{ formatSize(item.chars) }} · {{ item.span_count }} 个索引跨度</small></div>
...
.overview-grid, .overview-grid * { box-sizing: border-box; }
.overview-grid { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 32px 44px; width: 100%; max-width: 100%; height: 100%; max-height: 100%; min-width: 0; min-height: 0; overflow-x: hidden; overflow-y: auto; padding: 26px 30px 48px; }
section { min-width: 0; } .scroll-section, .sources-section { display: grid; grid-template-rows: auto minmax(0, 1fr); min-height: 260px; max-height: min(48vh, 520px); overflow: hidden; } .section-list { width: 100%; min-width: 0; min-height: 0; overflow-x: hidden; overflow-y: auto; overscroll-behavior: contain; scrollbar-gutter: stable; } .sources-section { grid-column: 1 / -1; }
...
.source-row { grid-template-columns: minmax(0, 1fr) max-content; gap: 16px; width: 100%; max-width: 100%; min-width: 0; overflow: hidden; }
...
code { min-width: 0; color: #999; font-size: 11px; white-space: nowrap; } .empty { padding: 18px 6px; color: #999; font-size: 13px; }
