<template>
    ...
    >
      <el-col :offset="2" :span="searchInputWidth" class="search-col">
        <SpecialInput
          ...
          :date-tag-value="dateTagValue"
          @cleanTag="cleanTag"
          @input="searching"
          :placeholder="$t('input.mockRecord.placeholder')"
          @clearData="clearDataFromSearch"
          @searchTimeString="searchTimeString"
          ...
          <transition-group name="fade" appear>
            <tr v-for="item in d.list" :key="item">
              ...
              <td
                ...
                @click="copy('http://' + item.server_host + item.path)"
              >
                <el-tooltip effect="light" placement="left">
                  ...
                  >
                  <div>{{ item.path }}</div></el-tooltip
                >
              </td>
              <td class="active-row long-td">
                <div>{{ item.origin_ip }}</div>
              </td>
              <td class="active-row long-td">
                <div>{{ tools.getLocaleDateTime(item.add_time, false) }}</div>
              </td>
              <td style="width: 15%">
                <div class="action-td action-view">
                  <CButton @click="showMockDetailFn(item.id)"
  ...
  <CommonDialog
    :dialog="showDialog"
    @closed="closedDialog"
    :headerText="t('project.MockRecord.dialogHeader')"
  >
    <MockDetail :data="detailData"></MockDetail>
  </CommonDialog>
</template>
...
<style lang="scss">
.action-view .customization-button .el-icon svg {
  width: 0.7em;
  height: 0.7em;
}
</style>
