      ...
      <el-table-column label="字段名" min-width="40%">
        <template #default="scope">
          <el-row style="width: 100%">
            <el-col :span="21">
              <el-select v-model="scope.row.name" filterable placeholder="参数名" :empty-values="[null, undefined]"
                :value-on-clear="null" @change="changeHeaderValue" :filter-method="filterMethodHandle"
                @blur="handleBlur($event, scope.row)" @focus="handleFocus($event, scope.row)">
                <el-option v-for="item in options_value" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-col>
          </el-row>
        </template>
      ...
      <el-table-column label="请求值">
        <template #default="scope">
          ...
          <div v-else class="core-value">
            <div style="width: 100%">
              <CodeMirror :canVar="props.canVar" v-model="scope.row.default" :enableNewLine="false"
                :interface_id="interface"></CodeMirror>
            </div>
          </div>
        </template>
      </el-table-column>
        ...
        <template #default="scope">
          <div style="display: flex;align-items: center;gap:3px" class="other-action">
            <motion.div :while-hover="{ scale: 1.05 }" :while-press="{ scale: 0.9 }"
              ...
              <el-icon @click="addNearNode(scope.row)" size="16" class="action-icon action-icon-plus" color="#139659">
                <CirclePlus />
              </el-icon>
            </motion.div>
            <motion.div :while-hover="{ scale: 1.05 }" :while-press="{ scale: 0.9 }"
