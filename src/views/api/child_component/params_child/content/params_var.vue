<template>
  ...
  <div class="show-content">
    <el-row>
      <el-col :span="24"
        ><span style="font-weight: 500; font-size: 13px">全局参数</span></el-col
      >
    </el-row>
    <div v-if="filter_global_params.length === 0" style="width: 100%;display: flex;justify-content: center;align-items: center;"><Empty></Empty></div>
    
    <el-row v-if="filter_global_params.length > 0" class="params-row" v-for="(item, index) in filter_global_params" :key="index" @click="set_params_name(item.name)">
      ...
      <el-col :span="12" style="display: flex; justify-content: end"
        ...
        <el-tooltip placement="top" effect="light">
            ...
            <span style="margin-right: 5px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">{{item.value}}</span>
          </el-tooltip>
        </el-col
      >
    </el-row>
    ...
    <div v-if="filter_env_params.length === 0" style="width: 100%;display: flex;justify-content: center;align-items: center;"><Empty></Empty></div>
    <el-row v-if="filter_env_params.length > 0" class="params-row" v-for="(item, index) in filter_env_params" :key="index" @click="set_params_name(item.name)">
      <el-col :span="24"><span style="margin-left: 5px">{{ item.name }}</span></el-col>
    </el-row>
  </div>
      ...
      >
        <el-row style="height: 100%">
          <el-col
            ...
            :span="11"
            ><span style="font-size: 12px; color: black">{{ item.function_sign }}</span></el-col
          >
          <el-col
...
const global_params = [
  {
    name: "name",
    value: "http://localhost:3000/home/main/project/interface/project/42",
  },
  {
    name: "age",
    value: "18",
...
onMounted(() => {
  filter_global_params.value = global_params
  filter_env_params.value = env_params
})

// 监听数组变化
watch([process_list, params], ([newList1, newList2], [oldList1, oldList2]) => {
  generation_expression()
  const foundItem = global_params.find(item => item.name === params.value);
  console.log(foundItem);
  if (foundItem) {
    generation_preview(foundItem)
  } else {
    preview.value = ""
  }
}, { deep: true });

const filter_global_params:any = ref([])
...
function change_params_name() {
  filter_global_params.value = global_params.filter((item:any) => {
    return item.name.indexOf(params.value) != -1
  })
  filter_env_params.value = env_params.filter((item:any) => {
    return item.name.indexOf(params.value) != -1
  })
  emit('reload_height')
}
function set_params_name(name:string) {
  params.value = name
  change_params_name()
}
...
function generation_preview(item:any) {
  ...
  process_list.value.forEach((process:any) => {
    const function_sign = modifyFunctionCalls(process.function_sign, input_value)
    const dynamicFunc = new Function('InnerFunction', `return InnerFunction.${function_sign};`);
...
function modifyFunctionCalls(functionCallStr:string, value:string | number) {
    ...
    return functionCallStr.replace(/([a-zA-Z0-9]+)\(([^)]*)\)/g, (match, fnName, args) => {
        const newArgs = args.trim() === '' ? input_value : `${input_value}, ${args.trim()}`;
