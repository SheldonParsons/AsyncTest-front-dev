<template>
  ...
  >
    <template #header="{ close }">
      <div class="amind-dialog-header">
        <div>
          <h3>选择 amind 文件</h3>
          <p>先选择 AsyncTest 项目，再从文件模块中勾选 `.amind` 文件。</p>
        </div>
        <button class="dialog-close" type="button" @click="close">×</button>
      </div>
    </template>

    <div class="amind-dialog-toolbar">
    ...
    <template #footer>
      <div class="amind-dialog-footer">
        <button class="footer-btn footer-btn-light" type="button" @click="handleClose">取消</button>
        <button class="footer-btn footer-btn-primary" type="button" @click="submitSelection">加入当前列表</button>
      </div>
    </template>
  </el-dialog>
</template>
...
function submitSelection() {
  ...
  if (!selectedFiles.length) {
    window.$toast({ title: "请至少选择一个 amind 文件", type: "warning" });
    return;
  }

  const project = props.projects.find((item) => item.value === selectedProjectId.value) ?? null;
  emit("confirm", selectedFiles, project);
  filesPageRef.value?.clean_select?.();
  emit("update:modelValue", false);
}
</script>
