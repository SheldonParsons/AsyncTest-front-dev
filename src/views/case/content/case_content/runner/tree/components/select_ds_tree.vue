    ...
    <el-tree class="api-tree no-scroll" id="api-tree" ref="treeRef" style="margin-top: 10px" :data="dataSource"
      ...
      <template #default="{ node, data }">
        ...
        " class="tree-node g-unselect">
          <el-icon v-if="data.child_type !== 4" :size="8" color="#606266" :class="node.expanded ? 'private-icon icon-expanded' : 'private-icon'
...
defineExpose({ filterText,getCheckedNodes })
...
async function load_tree(search_range = [0, 1, 4]) {
  loading.value = true;
  dataSource.value = [];
  const data = {
