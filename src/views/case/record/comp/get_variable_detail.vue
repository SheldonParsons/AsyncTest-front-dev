<template>
    ...
    <div class="title">提取标志</div>
    <MarkDown :data="`\`\`\`json\n${extract_info.extract_tag}\n\`\`\``"></MarkDown>
    <div class="Text">
        <div>{{ extract_info.name }}</div>
        <div>{{ variable_range_mapping[extract_info.save_type] }}</div>
        <div>{{ extract_source_mapping[extract_info.source] }}</div>
        <div>{{ extract_range_mapping[extract_info.extract_range] }}</div>
    </div>
</template>
...
const extract_info = computed(() => JSON.parse(props.data.extract_info))
...
const extract_source_mapping:any = {
    0: '响应体Body',
    1: '响应头Headers',
    2: 'Cookies',
    3: '响应时间'
...
const extract_range_mapping:any = {
    0: '整体数据',
    1: 'JsonPath',
    2: '正则表达式',
    3: 'XPath'
...
function get_variable_value() {
    return `\`\`\`json\n${extract_info.value.extract_value}\n\`\`\``
}
