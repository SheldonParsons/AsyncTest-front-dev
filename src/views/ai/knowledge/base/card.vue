<template>
  <div class="card" data-state="#about">
    <div class="card-header" @click="enter_document_list" style="cursor: pointer;">
    ...
    <div class="card-main">
      <div class="card-section is-active" id="about">
        <div class="card-content">
          <div class="card-subtitle">关于该知识库</div>
          <p class="card-desc">{{ desc }}</p>
        </div>
        <button class="edit-btn" @click="editProviderAction">更多设置</button>
        <button @click="showDeleteConfirmDialog = true" class="delete-btn">删除知识库</button>
      </div>
      <div class="card-section" id="contact">
        <div class="card-content">
          <div class="card-subtitle">知识库详情</div>
          <div class="card-contact-wrapper">
            ...
            >
              <el-col :span="8"
                ><span style="font-size: 14px; font-weight: 600">最近编辑：</span></el-col>
              <el-col :span="16">{{new Date(update_time).toLocaleString()}}</el-col>
            </el-row>
          </div>
        </div>
      </div>
      <div class="card-buttons">
        ...
        <button @click="handleButtonClick" data-section="#contact">更多信息</button>
      </div>
    </div>
  </div>
  <el-dialog
    ...
    style="padding: 20px;"
  >
    <span>删除知识库后，关联该知识库的应用将无法再使用该知识库，所有的提示配置和文档都将被永久删除。</span>
    <template #footer>
...
import { ref } from "vue";
import { deleteKnowledge } from "@/api/ai/index";
...
function enter_document_list() {
  const params = { project: Number(route.params.project), knowledge: props.knowledge };
...
function deleteProviderAction() {
  const data = {};
  deleteKnowledge(props.knowledge, data).then((res: any) => {
    if (res.msg) {
