...
import { updateApp } from "@/api/ai/index";
...
function update_provider() {
  ...
  data.append("remark", providerDesc.value);
  updateApp(props.provider, data).then((res: any) => {
...
function checking() {
  if (imageUrl.value === "") {
    tools.message(t("请上传应用图标"), proxy);
    return false;
  }
  if (providerName.value === "") {
    tools.message(t("请输入应用名称"), proxy);
    return false;
  }
  if (providerDesc.value === "") {
    tools.message(t("请输入应用描述"), proxy);
    return false;
  }
  return true;
}
