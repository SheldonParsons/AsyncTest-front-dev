  ...
  >
    ...
    <template #header="{ close, titleId, titleClass }">
      <el-row>
        ...
        >
          <h4 style="margin-bottom: 0px">编辑提供商插件</h4>
        </el-col>
      </el-row>
...
import { editProviderTool } from "@/api/ai/index";
...
watch(
  ...
  (val) => {
    ...
    plugin_name.value = prop.item.name;
    plugin_desc.value = prop.item.description;
    plugin_url.value = prop.item.url;
    plugin_method.value = prop.item.method;
    tableData.value = JSON.parse(prop.item.parameters);
  }
);
...
function create_provider() {
  ...
  editProviderTool(prop.item.id,data).then((res: any) => {
    console.log(res);
