<template>
  ...
  >
    ...
    >
      <Card
        ...
        @edit_plugin="editPluginAction"
        :tools="item.tools"
        :provider="item.id"
        :title="item.name"
        :subtitle="item.trademark"
  ...
  <AddProviderDialog @reload="clearDataFromSearch"></AddProviderDialog>
  <EditProviderDialog
  ...
  ></EditProviderDialog>
  <AddPluginDialog
    :showDialog="showAddPluginDialog"
    @close="closePluginDialog"
    :provider="current_provider"
  ></AddPluginDialog>
  <EditPluginDialog
    :showDialog="showEditPluginDialog"
    :item="current_plugin_item"
    @reload="clearDataFromSearch"
    @close="showEditPluginDialog = false"
  ></EditPluginDialog>
</template>
<script setup lang="ts">
import { ref, reactive, onMounted, getCurrentInstance } from "vue";
import Card from "./card.vue";
import AddProviderDialog from "./add_provider_dialog.vue";
import EditProviderDialog from "./edit_provider_dialog.vue";
import AddPluginDialog from "./add_plugin_dialog.vue";
import EditPluginDialog from "./edit_plugin_dialog.vue";
import SpecialInput from "@/components/common/input/specialInput.vue";
import { getProviderList } from "@/api/ai/index";
...
function editProviderAction(provider_id: number) {
  d.list.forEach((item: any) => {
    if (parseInt(item.id) === provider_id) {
      ...
      current_provider_trademark.value = item.trademark;
      current_provider_id.value = item.id;
      current_provider_headers.value = JSON.parse(item.headers);
      state.setMessage("editprovider" + Math.random().toString());
    }
  });
}
...
async function getProviderListAction(
  ...
  return getProviderList(data).then((res: any) => {
