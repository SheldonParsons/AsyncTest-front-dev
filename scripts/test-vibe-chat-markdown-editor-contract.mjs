...
assert.match(composerSource, /const text = textOverride \?\? inputEl\.value\?\.getMarkdown\?\.\(\) \?\? editorValue\.value[\s\S]*if \(!text\.trim\(\) && !\(props\.localMode && outgoingFiles\.length\)\) return/)
...
assert.match(composerSendSource, /const base = \(text \|\| ''\)\.trim\(\)[\s\S]*if \(!base && !fileList\.length\) return/)
