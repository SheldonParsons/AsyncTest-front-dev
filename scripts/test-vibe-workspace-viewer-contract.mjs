...
assert.match(indexSource, /function infoRailViewerOpenOptions\(\): WorkspaceWindowOpenOptions/)
assert.match(indexSource, /autoCollapseInfoRail:\s*!workspaceWindowRequestedOpen\.value[\s\S]*!workspaceWindowOpen\.value[\s\S]*!workspaceWindowLayoutActive\.value/)
assert.match(indexSource, /function openInfoRailChange\(item: KnowledgeCommitSummary\)[\s\S]*openWorkspaceChange\(item, infoRailViewerOpenOptions\(\)\)/)
assert.match(indexSource, /function openInfoRailFile\(file: RecentSessionFile\)[\s\S]*openWorkspaceFile\(file, activeSessionId\.value, infoRailViewerOpenOptions\(\)\)/)
...
assert.match(indexSource, /\.conversation-info-rail-slot\.workspace-active\.returning:not\(\.collapsed\)\s*\{[^}]*flex-basis:\s*var\(--conversation-info-rail-outer-width\);/)
assert.match(workspaceSource, /<header v-if="tabs\.length" class="workspace-head">/)
