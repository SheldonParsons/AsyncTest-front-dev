...
assert.match(indexSource, /sourceCitationHasReadableRange\(source\)[\s\S]*loadWorkspaceSourceFragment\([\s\S]*loadWorkspaceKnowledgeSource\(/)
assert.match(indexSource, /sourceCitationViewerIdentity\(reference\)/)
assert.match(indexSource, /getKnowledgeSource\(projectId, sourceId\)/)
assert.match(indexSource, /kind:\s*'knowledge-citation'/)
assert.equal((indexSource.match(/const source = payload\.source/g) || []).length, 2)
assert.match(indexSource, /content:\s*source\.content\s*\|\|\s*''/)
...
assert.doesNotMatch(indexSource, /getVibeSessionSource/)

assert.match(apiSource, /export interface KnowledgeSourceDetail/)
assert.match(apiSource, /export function getKnowledgeSource\(/)
assert.doesNotMatch(apiSource, /VibeSessionSource|getVibeSessionSource/)
