...
const viewerPolicyPath = path.join(root, 'src/views/electron_views/vibe/knowledge/workspaceViewerPolicy.ts')
...
const viewerPolicy = await importTs(viewerPolicyPath)
...
assert.equal(viewerPolicy.workspaceInlineFileContent({ content: '正文', text: '旧正文' }), '正文')
assert.equal(viewerPolicy.workspaceInlineFileContent({ text: '旧正文' }), '旧正文')
assert.equal(viewerPolicy.workspaceInlineFileContent({ content: '' }), '')
assert.equal(viewerPolicy.workspaceInlineFileContent({}), null)
assert.match(viewSource, /const inlineContent = workspaceInlineFileContent\(file\)/)
assert.match(viewSource, /if \(inlineContent === null\) void loadWorkspaceFile\(id\)/)
assert.match(viewSource, /const downloadUrl = String\(current\.file\.download_url \|\| ''\)\.trim\(\)/)
assert.match(viewSource, /downloadVibeSessionEventAttachment\([\s\S]*downloadUrl/)
