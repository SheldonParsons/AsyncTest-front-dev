...
assert.match(viewSource, /:floating="workspaceWindowLayoutActive"/)
assert.match(railSource, /:class="\{ collapsed, floating \}"/)
assert.match(railSource, /\.conversation-info-rail\.floating\s*\{[^}]*position:\s*absolute;[^}]*right:\s*calc\(var\(--workspace-window-width\) \+ 10px\);[^}]*flex:\s*none;[^}]*width:\s*min\(324px,\s*calc\(100% - var\(--workspace-window-width\) - 20px\)\);/)
assert.match(railSource, /\.conversation-info-rail\.floating\.collapsed\s*\{[^}]*flex-basis:\s*0;[^}]*width:\s*0;/)
...
assert.equal((railTemplate.match(/@click="showComingSoon"/g) || []).length, 2)
