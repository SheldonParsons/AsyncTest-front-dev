...
assert.match(disclosureSource, /watch\(bodyVisible, scheduleMessageRender/)
assert.match(disclosureSource, /watch\(\(\) => props\.steps, scheduleMessageRender/)
assert.match(disclosureSource, /renderedMessages\.value = next[\s\S]*emit\('layout-change'\)/)
...
assert.match(viewSource, /function flushElectronDeltaProjection\(\)[\s\S]*scrollBottomIfFollowing\(\)/)
...
assert.equal((viewSource.match(/:render-markdown="renderStreamingMarkdown"/g) || []).length, 5)
assert.match(disclosureSource, /v-html="messageHtml\(step\)"/)
assert.match(disclosureSource, /STREAMING_PROCESS_RENDER_DELAY_MS = 100/)
assert.match(disclosureSource, /messageCache\.get\(key\)/)
assert.doesNotMatch(disclosureSource, /enhanceCopyableCodeBlocks|copyable-code|copy-code/)
...
assert.match(viewSource, /ELECTRON_DELTA_PROJECTION_DELAY_MS = 50/)
assert.match(viewSource, /context\.ephemeralText = \(context\.ephemeralText \+ text\)\.slice\(-2_000_000\)[\s\S]*scheduleElectronDeltaProjection\(context\)/)
